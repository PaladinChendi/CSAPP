...
**English:** The `struct hostent` structure is as follows:

**简体中文：** struct hostent结构如下：
```c
struct hostent {
char *h_name;
char **h_aliases;
...
**English:** `char *h_name` is the canonical name of the host. For example, the canonical name of `www.google.com` is actually `www.l.google.com`.<br/>
`char **h_aliases` are the aliases of the host. `www.google.com` is itself an alias of google. Sometimes a host may have several aliases; these are just extra names the site chooses so that users can remember them more easily.<br/>
`int h_addrtype` is the type of the host's IP address, either IPv4 (`AF_INET`) or IPv6 (`AF_INET6`).<br/>
`int h_length` is the length of the host's IP address.<br/>
`int **h_addr_lisst` is the host's IP address; note that it is stored in network byte order. Never print this directly with `printf` using a `%s` argument — there will be problems. So when you actually need to print the IP, you must call `inet_ntop()`.<br/><br/>
The solution is to write a wrapper function that uses a mutex to protect a deep copy of the `hostent` structure, and store the return value in a local variable to make it thread-safe.

**简体中文：** char *h_name 表示的是主机的规范名。例如 www.google.com 的规范名其实是 www.l.google.com 。<br/>

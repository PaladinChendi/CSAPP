**English:** Two implementations are provided; they differ in the details of the circular queue. In version 1, when the queue is full it returns 403 directly; in version 2, when the queue is full the thread blocks and waits.

**简体中文：** 写了两个版本的实现，区别在于循环队列的实现细节不同，版本1队列满之后直接返回403，版本2队列满之后线程阻塞等待
<br/>
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        *connfdp = Accept(listenfd, (SA *)&clientaddr, &clientlen); //line:netp:tiny:accept
        ...
        if(!enqueue(*connfdp)) {
            clienterror(*connfdp, NULL, "403", "Forbidden",
                        "Tiny has reached the max number of connecitons");
        }
    }
}
/* $end tinymain */
...
int enqueue(int fd) {
    ...
    if (head == (tail + 1) % BUFSIZE) {
        sem_post(&mutex); //这里不做V操作，线程直接死锁饿死
        return 0;
    }
    
    circularQueue[tail] = fd;
    tail = (tail + 1) % BUFSIZE;
    sem_post(&mutex);

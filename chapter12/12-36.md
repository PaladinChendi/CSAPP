**English:** Note that after `doit` returns, `connfd` must be closed; otherwise the connected descriptor stays open and `select` keeps returning (it returns once for each connected descriptor that was not closed), causing the program to keep processing those descriptors and fall into an infinite loop.

**简体中文：** 要注意的是调用完doit之后应该把connfd关闭掉，不然连接描述符一直开着，select就会一直返回（有多少连接描述符没关就返回多少），导致程序一直在处理这些描述符进入到死循环。
```c
/* $begin tinymain */
/*
...
/*
 * init_pool - init the file descriptor pool
 */
/* $begin init_pool */
void init_pool(int listenfd, pool *p)
{
...
/*
 * add_client - add connected descriptor to the pool
 */
/* $begin add_client */
void add_client(int connfd, pool *p)
{
    ...
    for (i = 0; i < FD_SETSIZE; i++) /* Find an available slot */
        if (p->clientfd[i] < 0) {
            ...
            printf("add clinet[fd:%d] to pool\n", connfd);
            break;
        }
...
/*
 * check_clients - handle the requests and update the pool
 */
/* $begin check_clients */
void check_clients(pool *p)
{
    ...
    for (i = 0; (i <= p->maxi) && (p->nready > 0); i++) {
        ...
        if (FD_ISSET(connfd, &p->ready_set)) {
            p->nready--;
            printf("handle clinet[fd:%d] request\n", connfd);
            doit(&rio, connfd);
            printf("close clinet[fd:%d]\n", connfd);
            Close(connfd);

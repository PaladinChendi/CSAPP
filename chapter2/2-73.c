...
int saturating_add(int x, int y) {
    ...
    * thanks for https://dreamanddead.gitbooks.io/csapp-3e-solutions/content/chapter2/2.73.html
    */
    //x和y的符号位为0，并且sum的符号位为1，这种情况可以判断是正溢出
    int pos_over = !(x & sig_mask) && !(y & sig_mask) && (sum & sig_mask);
    //x和y的符号位为1，并且sum的符号位为0，这种情况可以判断是负溢出

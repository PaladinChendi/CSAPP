...
**English:** The point to note is that the book only gives the load-forwarding example for the combination of `mrmovq 0(%rcx), %rdx` and `pushq %rdx`. In fact, the combination of `mrmovq 0(%rcx), %rdx` and `rmmovq %rdx,0(%rcx)` also fits the load-forwarding case. So the final HCL is as follows:

**简体中文：** 要注意的地方是书上只举了mrmovq 0(%rcx), %rdx和pushq %rdx组合加载转发的例子。其实mrmovq 0(%rcx), %rdx和rmmovq %rdx,0(%rcx)组合也符合加载转发的情况。所以最终HCL如下:<br/>
e_valA = [<br/>
	E_icode in { IPUSHQ, IRMMOVQ } && E_srcA == M_dstM : m_valM;<br/>
	1 : E_valA;<br/>
]<br/>
...
bool F_stall =
	...
	E_icode in { IMRMOVQ, IPOPQ } && E_dstM in { d_srcA, d_srcB } 
	 && !(E_icode in { IPUSHQ, IRMMOVQ } && E_srcA == M_dstM) ||

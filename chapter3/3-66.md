...
This statement computes the next row's element to accumulate. The number of bytes to move from column b of row a to column b of row a+1 equals exactly the byte length of one row, so NC(n) = 4*n + 1.

**简体中文：** 说明1:<br>
可以看到这条语句就是计算下一行要累加的数据，而从第a行第b列移动到从第a+1行第b列要移动的字节长度，正好等于一行的字节长度，所以NC(n) = 4*n + 1<br>

**English:** Explanation 2:
%rdx holds the value of i. We compare the value of i with %rdi, where %rdi equals 3n. Then, based on `for(i=0;i<NR(n);i++)`, we get NR(n) = 3*n.
